extends Node2D
## Mapa: La Azotea — Techo del edificio, cielo abierto
## Solo conexión: Sur → Laboratorio (escalera de bajada)
## No hay salida al norte/este/oeste — es el techo

const TILE_W := 64
const TILE_H := 32
const PARED_H := 20

# Azotea: espacio abierto con bordes peligrosos y una escalera al sur
# tile 6 = puerta sur (bajada al Laboratorio)
const MAPA := [
	[2, 2, 2, 2, 2, 2, 2, 2, 2, 2],   # borde norte — sin salida
	[2, 4, 1, 1, 1, 1, 1, 1, 4, 2],   # bordes peligrosos (caída)
	[2, 1, 1, 3, 1, 1, 3, 1, 1, 2],   # acento: antenas
	[2, 1, 3, 1, 1, 1, 1, 3, 1, 2],
	[2, 1, 1, 1, 1, 1, 1, 1, 1, 2],
	[2, 1, 1, 1, 3, 3, 1, 1, 1, 2],   # acento: caja de agua
	[2, 1, 1, 1, 1, 1, 1, 1, 1, 2],
	[2, 4, 1, 1, 1, 1, 1, 1, 4, 2],   # bordes peligrosos
	[2, 2, 2, 2, 6, 6, 2, 2, 2, 2],   # fila 8 — escalera sur (2 tiles anchos)
]

const DESTINOS := {6: "sur"}
const LABELS   := {6: "⬇️ Laboratorio"}

# Colores azul-gris: cielo nublado, concreto expuesto
const C_PISO    := [Color(0.22,0.24,0.28), Color(0.14,0.16,0.19), Color(0.10,0.12,0.15)]
const C_PARED   := [Color(0.30,0.32,0.36), Color(0.20,0.22,0.25), Color(0.14,0.16,0.18)]
const C_ACENTO  := [Color(0.20,0.55,0.80), Color(0.13,0.35,0.52), Color(0.09,0.25,0.38)]
const C_PELIGRO := [Color(0.55,0.45,0.10), Color(0.35,0.28,0.06), Color(0.25,0.20,0.04)]
...
func get_spawn_desde(_desde: String) -> Vector2:
	# Siempre spawneamos cerca de la escalera (sur del mapa)
	return get_tile_pos(4, 6)
